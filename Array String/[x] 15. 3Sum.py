...
def threeSum(nums: List[int]) -> List[List[int]]:
	...
	for first in range(len(nums)-2):
		for second in range(first+1, len(nums)):
			searching = -1 * (nums[first] + nums[second])
			print(searching)
			if searching in nums[second+1:]:
				third = nums[second+1:].index(searching) + second + 1
				duplicate = False
				
				for comb in res:
					if nums[first] in comb and nums[second] in comb and nums[third] in comb:
						duplicate = True
						break
				
				if not duplicate:
					subarr = [nums[first]] + [nums[second]] + [nums[third]]
					res.append(subarr)
